...
const Assignments = () => {
    ...
    dueDate: ''
     ...
     const handleDelete = async (assignmentId) => {
         ...
         try {
           const { data } = await axios.delete(`${backendUrl}/api/educator/delete-assignment/${assignmentId}`, {
  ...
  return assignmentsLoading ? (<Loading /> ): (
//         <div className="max-w-4xl bg-white p-6 rounded shadow">
//           <h2 className="text-lg font-medium mb-4">Assignments</h2>
//           <table className="w-full table-auto">
//             <thead>
//               <tr><th>Course</th><th>Chapter</th><th>Title</th><th>Due Date</th><th>Actions</th></tr>
//             </thead>
//             <tbody>
//               {assignments.map(a => (
//                 <tr key={a.assignmentId} className="border-b">
//                   <td>{courses.find(c => c._id === a.courseId)?.courseTitle || ''}</td>
//                   <td>{chapters.find(ch => ch.chapterId === a.chapterId)?.chapterTitle || a.chapterId}</td>
//                   <td>{a.title}</td>
//                   <td>{new Date(a.dueDate).toLocaleDateString()}</td>
//                   <td>
//                     <button onClick={() => handleEdit(a)} className="text-blue-600 mr-2">Edit</button>
//                     <button onClick={() => handleDelete(a.assignmentId)} className="text-red-600">Delete</button>
//                   </td>
//                 </tr>
//               ))}
//             </tbody>
        ...
        <div className="flex flex-col items-center max-w-4xl w-full overflow-hidden rounded-md bg-white border border-gray-500/20">
          <table className="md:table-auto table-fixed w-full overflow-hidden">
            ...
            <tbody className="text-sm text-gray-500">
              {assignments.map((a) => (
                <tr key={a.assignmentId} className="border-b border-gray-500/20">
                  <td className="px-4 py-3 truncate">
                    {courses.find((c) => c._id === a.courseId)?.courseTitle || a.courseId}
                  </td>
                  <td className="px-4 py-3 truncate">
                    {chapters.find((ch) => ch.chapterId === a.chapterId)?.chapterTitle || a.chapterId}
                  </td>
                  ...
                  <td className="px-4 py-3 truncate space-x-2">
                    ...
                    <button onClick={() => handleDelete(a.assignmentId)} className="text-red-600 hover:underline">
                      Delete
                    </button>
                  </td>
                </tr>
