...
const AddQuizz = () => {
  ...
  if (!form.courseId || !form.chapterId) {
    return toast.error("Select a course and chapter first");
  }
  ...
  try {
    ...
    const token = localStorage.getItem("token");
    const { data } = await axios.post(
      `${backendUrl}/api/educator/quiz/generate-ai`,
      {
    ...
    if (data.success) {
      setForm((prev) => ({
        ...prev,
        quizQuestions: data.quizQuestions, // expecting array of { question, options, correctAnswer }
      }));
      toast.success("Quiz generated by AI");
    } else {
      toast.error(data.message || "Failed to generate quiz");
    }
  } catch (err) {
    toast.error(err.message || "Error while generating quiz");
  } finally {
    setLoading(false);
  }
};
